...
class ComunicacaoService:
    ...
    def _enviar_whatsapp_coroinha(coroinha, texto: str) -> bool:
        enviado = False
        for resp in coroinha.responsaveis.all():
            tel = resp.whatsapp or resp.telefone
            if tel and EnvioService.enviar_whatsapp(tel, texto):
                enviado = True
        if coroinha.telefone and EnvioService.enviar_whatsapp(coroinha.telefone, texto):
            enviado = True
        return enviado
